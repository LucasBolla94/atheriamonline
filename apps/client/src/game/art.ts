...
async function loadVenueProps(): Promise<HTMLCanvasElement> {
  const image = await load('/art/city-furniture.png');
  const out = canvas(512, 576);
  out.getContext('2d')!.drawImage(image, 0, 0);
  return out;
}
