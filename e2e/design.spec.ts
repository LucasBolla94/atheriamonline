...
async function join(page: Page, prefix: string) {
  ...
  await expect(page.locator('.resident-card img')).toHaveAttribute('src', '/art/portrait-1.png');
  return name;
}
...
test('visual review: entry, city, saved looks and mobile portrait', async ({
  ...
  try {
    ...
    await page.getByRole('button', { name: 'Market gold' }).focus();
    await page.keyboard.press('Enter');
    await expect(page.getByRole('button', { name: 'Market gold' })).toHaveAttribute(
      'aria-pressed',
      'true',
    );
